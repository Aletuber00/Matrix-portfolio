...
// --- PATCH RESPONSIVE: ridimensiona il canvas se la finestra cambia ---
window.addEventListener('resize', function () {
    w = q.width  = window.innerWidth;
    h = q.height = window.innerHeight;
    // Resetta le colonne in modo che non puntino fuori schermo
    p = Array(Math.floor(w / 10)).join(1).split('');
});

// Blocca lo scroll di body durante intro/pioggia
    ...
    if (color === 'blue') {
        ...
        setTimeout(() => {
            ...
            setTimeout(function () {
                ...
                // Riabilita lo scroll quando il sito è visibile
...
function openSection(sectionName) {
    ...
    if (targetSection) {
        ...
        // Su mobile scrolla il terminale in vista
